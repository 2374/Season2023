...
public class ArmSoftLimits {
    ...
    public ArmEndEffectorState applyLimit(ArmEndEffectorState in) {
        ...
        var x = clipPos.x;
        var y = clipPos.y - Constants.ARM_SHOULDER_MOUNT_HEIGHT;
        // Ensure the input point is "reachable" by scaling it back
        // inside the unit circle of the max extension of the arm.
        double maxRadius = Constants.ARM_BICEP_LENGTH + Constants.ARM_FOREARM_LENGTH * 0.99999;
        double minRadius = Math.abs(Constants.ARM_BICEP_LENGTH - Constants.ARM_FOREARM_LENGTH) * 1.00001;
        double reqRadius = Math.sqrt(x * x + y * y);

        if (reqRadius == 0.0) {
            // user was silly, give up
            clipPos.x = minRadius;
            clipPos.y = 0.0 + Constants.ARM_SHOULDER_MOUNT_HEIGHT;
            clipPos.xvel = 0;
            clipPos.yvel = 0;
            ...
            clipPos.y = maxRadius * Math.sin(angle) + Constants.ARM_SHOULDER_MOUNT_HEIGHT;
            clipPos.xvel = 0;
            clipPos.yvel = 0;
            ...
            clipPos.y = minRadius * Math.sin(angle) + Constants.ARM_SHOULDER_MOUNT_HEIGHT;
            clipPos.xvel = 0;
            clipPos.yvel = 0;
        }
