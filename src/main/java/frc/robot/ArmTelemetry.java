...
import frc.robot.subsystems.expArm.ArmEndEffectorState;
import frc.robot.subsystems.expArm.Path.ArmPath;
import frc.robot.subsystems.expArm.ArmAngularState;
...
public class ArmTelemetry {
    ...
    private final MechanismRoot2d m_armPivot = m_mech2d.getRoot("ArmPivot", LEFT_MARGIN,
            Constants.ARM_SHOULDER_MOUNT_HEIGHT);
    private final MechanismLigament2d m_arm_tower = m_armPivot.append(
            new MechanismLigament2d("ArmTower", Constants.ARM_SHOULDER_MOUNT_HEIGHT, -90, 5, new Color8Bit(Color.kSilver)));
    ...
    private final MechanismLigament2d m_shoulder_ligament_act = m_armPivot.append(
            new MechanismLigament2d(
                    "Arm Shoulder Act",
                    Constants.ARM_BICEP_LENGTH,
                    -90,
                    3,
                    new Color8Bit(Color.kSteelBlue)));

    private final MechanismLigament2d m_elbow_ligament_act = m_shoulder_ligament_act.append(
            new MechanismLigament2d(
                    "Arm Elbow Act",
                    Constants.ARM_FOREARM_LENGTH,
    ...
    private final MechanismLigament2d m_shoulder_ligament_meas = m_armPivot.append(
            new MechanismLigament2d(
                    "Arm Shoulder Meas",
                    Constants.ARM_BICEP_LENGTH,
                    -90,
    ...
    private final MechanismLigament2d m_elbow_ligament_meas = m_shoulder_ligament_meas.append(
            new MechanismLigament2d(
                    "Arm Elbow Meas",
                    Constants.ARM_FOREARM_LENGTH,
                    Units.radiansToDegrees(0.0),
    ...
    private final MechanismLigament2d m_shoulder_ligament_des = m_armPivot.append(
            new MechanismLigament2d(
                    "Arm Shoulder Des",
                    Constants.ARM_BICEP_LENGTH / 3.0,
                    -90,
                    10,
                    new Color8Bit(Color.kLimeGreen)));

    private final MechanismLigament2d m_elbow_ligament_des = m_shoulder_ligament_meas.append(
            new MechanismLigament2d(
                    "Arm Elbow Des",
                    Constants.ARM_FOREARM_LENGTH / 3.0,
                    Units.radiansToDegrees(0.0),
                    10,
                    new Color8Bit(Color.kLimeGreen)));
    ...
    private ArmTelemetry() {
        ...
        if (Robot.isReal()) {
            ...
            m_shoulder_ligament_act.setLength(0);
            m_elbow_ligament_act.setLength(0);
        }
    }
    ...
    public void setActual(double shoulderAngleDeg, double elbowAngleDeg) {
        m_shoulder_ligament_act.setAngle(shoulderAngleDeg);
        m_elbow_ligament_act.setAngle(elbowAngleDeg);
    }
    ...
    public void setDesired(ArmEndEffectorState desPos, ArmAngularState desArmState) {
        ...
        m_shoulder_ligament_des.setAngle(desArmState.shoulderAngleDeg);
        m_elbow_ligament_des.setAngle(desArmState.elbowAngleDeg);
    ...
    public void setMeasured(ArmEndEffectorState measPos, ArmAngularState measArmState) {
        m_shoulder_ligament_meas.setAngle(measArmState.shoulderAngleDeg);
        m_elbow_ligament_meas.setAngle(measArmState.elbowAngleDeg);
